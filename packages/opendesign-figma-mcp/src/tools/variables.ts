...
export function registerVariableTools(server: McpServer, connector: FigmaConnector) {
  ...
  srv.tool("figma_get_variables", "Get all variables in the file", {
  ...
  srv.tool("figma_get_token_values", "Get resolved token values for a collection and mode", {
  ...
  srv.tool("figma_browse_tokens", "Browse design tokens with optional collection filter", {
  ...
  srv.tool("figma_create_variable", "Create a new variable in a collection", {
    ...
    const result = await connector.createVariable({ collectionId: params.collectionId, name: params.name, type: params.type, value: params.value })
    return { content: [{ type: "text" as const, text: JSON.stringify(result) }] }
  })

  srv.tool("figma_batch_create_variables", "Create multiple variables in one call (up to 100)", {
  ...
  srv.tool("figma_update_variable", "Update an existing variable value", {
    variableId: z.string().describe("Variable ID to update"),
    value: z.any().describe("New value"),
    modeId: z.string().optional().describe("Mode ID (uses default mode if omitted)"),
  }, async (params: any) => {
    const result = await connector.updateVariable({ variableId: params.variableId, value: params.value, modeId: params.modeId })
    return { content: [{ type: "text" as const, text: JSON.stringify(result) }] }
  })

  srv.tool("figma_batch_update_variables", "Update multiple variable values in one call (up to 100)", {
  ...
  srv.tool("figma_create_variable_collection", "Create a new variable collection", {
