...
export function registerCommentTools(server: McpServer, restClient: FigmaRestClient) {
  ...
  srv.tool("figma_get_comments", "Get comments on a Figma file", {
  ...
  srv.tool("figma_post_comment", "Post a comment on a Figma file", {
    fileKey: z.string().describe("The file key"),
    ...
    const coords = params.x !== undefined && params.y !== undefined ? { x: params.x, y: params.y, node_id: params.node_id } : undefined
    const result = await restClient.postComment(params.fileKey, params.message, coords)
    return { content: [{ type: "text" as const, text: JSON.stringify(result) }] }
  })

  srv.tool("figma_delete_comment", "Delete a comment", {
    fileKey: z.string().describe("The file key"),
