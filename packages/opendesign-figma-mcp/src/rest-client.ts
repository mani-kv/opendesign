/**
 * Typed Figma REST API client with OAuth token from env/config, retry logic, and comment endpoints.
 */
...
export class FigmaRestClient {
  ...
  private async deleteRequest<T = void>(path: string): Promise<T> {
    const token = await this.resolveToken()
    const res = await fetchWithRetry(
      `${this.baseUrl}${path}`,
      token,
  ...
  deleteComment(fileKey: string, commentId: string): Promise<void> {
    return this.deleteRequest(`/v1/files/${fileKey}/comments/${commentId}`)
  }
}

// ── Factory ───────────────────────────────────────────────────────────────────

export function createRestClient(): FigmaRestClient {
