...
figma.ui.onmessage = async (msg) => {
      ...
      try {
        ...
        figma.ui.postMessage({
          ...
          data: {
            ...
            variableCollections: collections.map(function (c) {
              return {
                id: c.id,
                name: c.name,
                key: c.key,
        ...
        try {
          var importResult = await Promise.race([
            ...
            new Promise(function (_, reject) {
              setTimeout(function () {
                reject(new Error("Import timed out after 15s — component may not be published to a team library"))
              }, 15000)
            }),
          ])
        ...
        if (!component) {
          try {
            var setResult = await Promise.race([
              ...
              new Promise(function (_, reject) {
                setTimeout(function () {
                  reject(new Error("ComponentSet import timed out after 15s"))
                }, 15000)
              }),
            ])
            ...
            if (setResult && setResult.type === "COMPONENT_SET") {
              console.log(
                '🌉 [Desktop Bridge] Imported component set "' +
                  setResult.name +
                  '" with ' +
                  setResult.children.length +
                  " variants",
              )
            ...
            console.log(
              "🌉 [Desktop Bridge] importComponentSetByKeyAsync also failed: " + setErrMsg + ", trying local...",
            )
          }
        }
      }
  ...
  // ============================================================================
    ...
    figma.clientStorage.setAsync("cloudConfig", { code: msg.code, timestamp: Date.now() }).catch(function () {
      /* non-critical */
    })
  }
  ...
  // ============================================================================
    ...
    try {
      ...
      console.log(
        "🌉 [Desktop Bridge] Lint complete: " + summaryObj.total + " findings across " + nodesScanned + " nodes",
      )
