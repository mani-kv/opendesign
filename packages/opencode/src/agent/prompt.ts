...
export namespace AgentPrompt {
  const log = Log.create({ service: "agent.prompt" })
  ...
  export const prompt = fn(PromptInput, async (input) => {
    ...
    return loop({ agentID: input.agentID })
  })
  ...
  function resume(agentID: string) {
    const s = state()
    if (!s[agentID]) return

    return s[agentID].abort.signal
  }

  export function cancel(agentID: string) {
    log.info("cancel", { agentID })
    const s = state()
    const match = s[agentID]
    if (!match) {
      SessionStatus.set(agentID, { type: "idle" })
      return
    }
    match.abort.abort()
    delete s[agentID]
  ...
  export const loop = fn(LoopInput, async (input) => {
    const { agentID, resume_existing } = input

    const abort = resume_existing ? resume(agentID) : start(agentID)
    if (!abort) {
      return new Promise<MessageV2.WithParts>((resolve, reject) => {
        const callbacks = state()[agentID].callbacks
        callbacks.push({ resolve, reject })
      })
    }

    using _ = defer(() => cancel(agentID))
    ...
    while (true) {
      ...
      let msgs = await MessageV2.filterCompacted(MessageV2.stream(agentID))
    ...
    SessionCompaction.prune({ agentID })
    for await (const item of MessageV2.stream(agentID)) {
      if (item.info.role === "user") continue
      const queued = state()[agentID]?.callbacks ?? []
  ...
  async function lastModel(agentID: string) {
    for await (const item of MessageV2.stream(agentID)) {
  ...
  export async function resolveTools(input: {
    ...
    for (const [key, item] of Object.entries(await MCP.tools())) {
      ...
      item.execute = async (args, opts) => {
        ...
        const attachments: Omit<MessageV2.FilePart, "id" | "agentID" | "messageID">[] = []
        ...
        if (part.type === "file") {
          ...
          switch (url.protocol) {
            ...
            case "file:":
              ...
              FileTime.read(input.agentID, filepath)
              return [
                {
                  messageID: info.id,
                  agentID: input.agentID,
  ...
  export async function command(input: CommandInput) {
    ...
    const taskModel = await (async () => {
      ...
      return await lastModel(input.agentID)
    })()
