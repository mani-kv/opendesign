...
- **Phase D modules** (added 2026-03-20): `design-system/registry` (SolidJS store for workspace-level DS CRUD — add/remove/setActive/updateTokens/updateComponents), `design-system/tokens` (tokensToCss, tokensToStylesheet, diffTokens), `design-system/components` (groupByPackage, searchComponents, componentImport), `git/branch` (agentBranchName slug generation, createBranchInfo, isValidBranchName, filterAgentBranches), `git/checkpoint` (createCheckpointInfo with cp_ IDs, sortCheckpoints, latestCheckpoint), `git/diff` (successMerge, conflictMerge, categorizeConflicts).
